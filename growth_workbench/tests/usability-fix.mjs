...
try {
  ...
  const generated = await send(`/api/topics/${firstTopic.topic_id}/generate`);
  check('默认单稿生成', generated.status === 201 && generated.data.version === 1);
  check('未配模型安全回落并评分', generated.data.generation_mode === 'template' && Number.isFinite(generated.data.style_score) && generated.data.lin_fallback_reason === 'llm_not_configured');
  ...
  const secondScript = await send(`/api/topics/${secondTopic.topic_id}/generate`);
  ...
  const cuts = await send('/api/themes/cuts', { theme: '球馆盈利' });
  ...
  const selectedCut = await send(`/api/topics/${four[0].topic_id}/generate`);
  ...
  const allDrafts = await send(`/api/themes/${cuts.data.theme_id}/generate-all`);
  ...
  const derived = await send(`/api/inspirations/${extracted.data.inspiration_id}/create-topic`);
