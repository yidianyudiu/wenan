...
try {
  ...
  check('模型硬禁词触发安全回落', guarded.status === 201 && guarded.data.generation_mode === 'template' && guarded.data.lin_fallback_reason.includes('style_hard_block') && !guarded.data.full_script.includes('评论区'));
...
await import('./flywheel-quality.mjs');
