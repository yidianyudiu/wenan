...
const { createCalibrationEvent, rebuildRevisionPairs, enrichCalibrationEvents, effectForRule, confidenceForRule, extractPreferenceCandidates, buildFeedbackFlywheel, syncFeedbackFlywheel } = await import('../src/feedback-flywheel.mjs');
...
test('T4 规则效果归因、持久化与置信度反映真实结果', async () => {
  ...
  const strictRuleId = 'pref_strict_effect_contract';
  const strictEvents = rows => rows.map(([style_score, decision, applied], index) => ({
    review_id: `strict_${index}`,
    decision,
    script_snapshot: { full_script: 'x', style_score, active_preference_rule_ids: applied ? [strictRuleId] : [] },
    revision_outcome: null,
    preference_candidates: []
  }));
  const strictEffect = rows => buildFeedbackFlywheel(strictEvents(rows), [{ rule_id: strictRuleId, dimension: 'custom', instruction: '严格归因测试', status: 'active' }])
    .rules.find(rule => rule.rule_id === strictRuleId).effect;
  const beneficial = strictEffect([
    [90, '直接拍', 1], [86, '直接拍', 1], [88, '修改后拍', 1],
    [70, '修改后拍', 0], [74, '不拍', 0], [72, '不拍', 0]
  ]);
  assert.deepEqual(beneficial, { applied_count: 3, accepted_after: 2, revised_after: 1, rejected_after: 0, avg_score_delta: 16, recurrence_rate: 0 });
  const harmful = strictEffect([
    [60, '不拍', 1], [64, '修改后拍', 1],
    [80, '直接拍', 0], [84, '直接拍', 0]
  ]);
  assert.deepEqual(harmful, { applied_count: 2, accepted_after: 0, revised_after: 1, rejected_after: 1, avg_score_delta: -20, recurrence_rate: 0 });
});
...
test('T5 生成留痕、统一评分与历史V1到V2配对可审计', async () => {
  ...
  const rebuilt = rebuildRevisionPairs([event], [v1, v2]);
  assert.equal(rebuilt[0].revision_outcome.script_id, 'v2');
  assert.equal(rebuilt[0].script_snapshot.full_script, '真实V1');
