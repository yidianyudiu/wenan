...
import { generateTopicCuts } from './src/topic-generator.mjs';
...
async function createScriptForTopic(topic, { mode = 'lin' } = {}) {
  const calibration = await feedbackGenerationContext(store);
...
async function api(req, res, url) {
  ...
  if (method === 'GET' && p === '/api/health') return json(res, 200, { ok: true, workbench: '0.1.3-openrouter-flywheel-01', content_os: adapter.status(), lin_style: styleRuntimeStatus(), feedback_flywheel: 'feedback_flywheel_v2', data_dir: dataDir });
  ...
  if (method === 'POST' && p === '/api/themes/cuts') {
    ...
    let cuts;
    try { cuts = generateTopicCuts(input.theme); }
    catch (error) { return json(res, 422, { error: error.message }); }
    const createdAt = now();
    const topics = cuts.map(cut => {
  ...
  if (method === 'POST' && /\/api\/inspirations\/[^/]+\/create-topic$/.test(p)) {
    ...
    const match = String(item.customer_problem || '').split(' / ')[0];
    const orientations = {
      '获客': { title: '球馆活动做了不少，客户到底从哪里来？', domain: '老馆运营 / 获客', thesis: 'BT-05' },
      '成交': { title: '客户已经到店了，为什么还是不愿意报名？', domain: '教学培训 / 成交', thesis: 'BT-02' },
      '续费': { title: '学员上了一段时间，为什么还是不愿意续费？', domain: '教学培训 / 续费', thesis: 'BT-02' },
      '利润': { title: '球馆看起来很忙，为什么最后还是没有利润？', domain: '老馆运营 / 利润', thesis: 'BT-01' },
      '竞争': { title: '球馆越来越多，客户凭什么选择你？', domain: '老馆运营 / 竞争', thesis: 'BT-05' },
      '团队': { title: '老板每天都在现场，为什么团队还是带不起来？', domain: '团队 / 组织', thesis: 'BT-03' },
      '新馆': { title: '新馆开业前，为什么不能只准备开业当天？', domain: '新馆经营 / 开业', thesis: 'BT-04' }
    };
    const orientation = orientations[match];
    if (!orientation) return json(res, 422, { error: '还不能确定球馆老板的真实经营问题，请先补充原内容或人工确认问题方向。' });
    const internal = adapter.searchInternal(orientation.title, 3);
    const topic = {
      topic_id: uid('topic'),
      title: orientation.title,
      customer_problem: `球馆老板当前面临的${match}问题，需要用搜羽内部经营经验给出原创答案。`,
      topic_domain: orientation.domain,
      brand_thesis_id: orientation.thesis,
      recommendation_reason: '由外部市场信号发现问题，再回到 Internal Intelligence 形成屿洁自己的经营答案。',
      knowledge_coverage: internal.map(entry => entry.title).join('；'),
      inspiration_source: 'External Inspiration + Internal Intelligence',
      external_inspiration_id: inspirationId,
      topic_score: 82,
      status: '待评估',
      solution_points: ['先解释经营结果为什么没有发生', '再定位客户旅程中的关键断点', '调用搜羽内部体系给出可执行方案'],
      problem_explanation: '外部内容只帮助发现市场正在讨论的问题，正式内容必须回到搜羽自己的经营判断和可追溯知识。',
      conclusion: '不是换词复述别人的观点，而是用搜羽真实做球馆的经验重新回答这个问题。',
      created_at: now()
    };
    const gate = topicOrientationGate(topic);
    if (gate.status !== 'PASS') return json(res, 422, { error: gate.reason, gate });
