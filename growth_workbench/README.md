...
状态：`GROWTH_WORKBENCH_V0.1_AUDIT_RECTIFICATION_02_READY`
...
历史人工审核会在启动时自动补齐为 `feedback_flywheel_v2` 快照。历史 V1→V2 配对由可独立验收的纯函数重建，且不会改写任何历史正文。当前已有的候选规则不会被自动启用，需由用户逐条确认。
...
当前自动化覆盖83项检查：27项 V0.1 可用性回归、13项林总风格规则自检、20项飞轮与语义质量测试、23项本地 OpenAI/OpenRouter 兼容接口端到端测试。新增覆盖严格的启用组/对照组效果分差、历史版本配对不改V1、近似重构证据警告、握拍主题不再误判利润、不相关知识不强行检索、外部全文注入与查看、残缺 JSON 停止落库、数字/经验背书门禁和语义素材忠实审计。
