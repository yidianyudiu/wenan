...
状态：`GROWTH_WORKBENCH_V0.1_OPENROUTER_FLYWHEEL_01_READY`
...
- 原 `composeBestScript()` 完整保留，未配密钥、断网、模型失败或命中平台硬禁词时自动安全回落；
...
当前默认模型为 OpenRouter `stealth/ox-alpha`，请求走官方 OpenAI 兼容端点 `https://openrouter.ai/api/v1/chat/completions`。首次配置时复制 `.env.example` 为本机 `.env`，只填写 `OPENROUTER_API_KEY` 后重启。密钥不会进入脚本、反馈数据、日志或备份。本次按用户明确要求生成的私有 ZIP 已内置 `.env`，不可公开分享。

模型请求默认使用 `reasoning.effort=low`、隐藏 reasoning 内容，并给成稿预留 6000 completion tokens。未配置密钥、断网、模型失败或命中硬禁词时，工作台仍安全回落到 V0.1 模板路径并标记原因。

隐私边界：工作台只向模型发送当前选题、脱敏后的检索片段、已人工确认的写作偏好和少量“直接拍”正样本。OpenRouter 的 Stealth 模型页面标明其当前上游提供方会保留 prompts/completions，因此不要在反馈或资料中加入不应发送给第三方的秘密信息。需要换模型时可修改 `LIN_LLM_MODEL`；接口层仍兼容其他 OpenAI 协议服务。
...
当前测试由三组组成：24项 V0.1 可用性回归、13项林总风格规则自检，以及22项本地 OpenAI/OpenRouter 兼容接口端到端测试，共59项。覆盖模型生成、风格分、CTA 白名单、硬禁词回落、敏感知识脱敏、V2 版本生成、反馈快照、人工确认后偏好注入、直接拍正样本与密钥不泄露。

真实 OpenRouter 探针可在已配置密钥的本机运行 `npm run openrouter:check`。它只输出服务商、实际模型、风格分和用量摘要，不打印提示词或成稿。本次私有交付 ZIP 包含本机 `.env`；未配置外部密钥时，自动化测试仍会使用本地模拟兼容端点验证完整链路。
