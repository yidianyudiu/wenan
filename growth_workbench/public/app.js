...
function settings() {
  ...
  return `<div class="settings-stack">${feedbackFlywheelSettings()}${creatorSettings()}${performanceSettings()}<section class="settings-section static"><div><b>系统状态</b><small>Content OS ${esc(state.system.state)} · Content Skill ${esc(state.system.content_skill)}</small><small>林总风格引擎：${lin.configured ? `${esc(lin.provider || '')} · ${esc(lin.model)} 已配置` : '未配置 API Key，当前自动使用模板回落'} · 敏感知识脱敏 ${lin.redactSensitive === false ? '关闭' : '开启'}</small></div></section><section class="settings-section static"><div><b>数据备份</b><small>备份工作台本地数据，不修改 Content OS 核心数据库。</small></div><button class="secondary" id="backup-btn">立即备份</button></section></div>`;
}
...
async function topicAction(action, id) {
  try {
    if (action === 'generate') {
      ...
      await refresh();
      setView('studio');
      flash(script.generation_mode === 'lin' ? `已生成1篇林总风格文案，风格匹配度 ${script.style_score}/100` : `模型未配置或不可用，已回落原模板并完成风格评分 ${script.style_score}/100`);
      return;
    }
