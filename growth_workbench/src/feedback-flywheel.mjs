...
// 纯函数：只给已有审核事件补充后继版本快照，不改写 V1 或任何历史稿正文。
// 单独导出，供迁移流程和质量回归复用，避免“已有 V2 但配对计数仍为 0”。
export function rebuildRevisionPairs(events = [], scripts = []) {
  return events.map(event => {
    if (event.decision !== '修改后拍' || event.revision_outcome?.script_id) return event;
    const revision = findHistoricalRevision(event, scripts);
    if (!revision) return event;
    return {
      ...event,
      revision_outcome: scriptSnapshot(revision),
      learning_status: 'paired_v1_to_revision'
    };
  });
}

export function enrichCalibrationEvents(events, scripts = [], topics = []) {
  return rebuildRevisionPairs(events, scripts).map(event => {
    ...
    return {
      ...
      revision_outcome: event.revision_outcome || null,
      learning_status: event.learning_status || (event.revision_outcome?.script_id ? 'paired_v1_to_revision' : 'captured_pending_rule_review'),
...
export function effectForRule(ruleId, dimension, events) {
  ...
  const notApplied = events.filter(event => !activeRuleIds(event).includes(ruleId));
  const appliedScores = applied.map(event => numericScore(event.script_snapshot?.style_score)).filter(score => score != null);
  const controlScores = notApplied.map(event => numericScore(event.script_snapshot?.style_score)).filter(score => score != null);
  const average = scores => scores.reduce((sum, score) => sum + score, 0) / scores.length;
  const delta = appliedScores.length && controlScores.length ? average(appliedScores) - average(controlScores) : null;
  ...
  return {
    ...
    avg_score_delta: delta == null ? null : Math.round(delta * 10) / 10,
