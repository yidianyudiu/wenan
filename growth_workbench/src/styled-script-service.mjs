...
function withStyle(record, script, scored, metadata) {
  return {
    ...
    generation_context: {
      ...
      external_inspiration: metadata.externalInspiration || null,
...
async function linResult(legacy, topic, opts = {}) {
  ...
  const unsafeBlocks = (result.lint.blocks || []).filter(item => ['B01', 'B02', 'B03', 'B04', 'B05'].includes(item.id));
  ...
  return {
    ...
    script: withStyle(legacy.script, result.script, { lint: result.lint, report: result.report }, {
      ...
      externalInspiration: result.trace?.external_inspiration || null,
      promptHash: result.trace?.prompt_hash || null
    })
  };
}

export async function generateStyledScript(topic, adapter, { mode = 'lin', preferenceRules = [], positiveExamples = [], externalMaterial = null } = {}) {
  const legacy = composeBestScript(topic, adapter);
  if (legacy.blocked) return legacy;
  if (mode === 'template') return templateResult(legacy, 'template_mode_selected', topic);
  if (!llmConfigured()) return { blocked: true, error: '大模型未配置，已停止生成；如需模板草稿必须明确选择模板模式' };
  const styled = await linResult(legacy, topic, { preferenceRules, positiveExamples, externalMaterial });
  if (!styled.ok) return { blocked: true, error: `大模型生成失败，未保存模板稿：${safeError(styled.error)}` };
  return { blocked: false, script: styled.script };
}

export async function reviseStyledScript(sourceScript, review, topic, adapter, version, { mode = 'lin', preferenceRules = [], positiveExamples = [], externalMaterial = null } = {}) {
  const legacy = composeRevision(sourceScript, review, topic, adapter, version);
  if (legacy.blocked) return legacy;
  if (mode === 'template') return templateResult(legacy, 'template_mode_selected', topic);
  if (!llmConfigured()) return { blocked: true, error: '大模型未配置，已停止改稿；原稿和历史版本保持不变' };
  ...
  if (!styled.ok) return { blocked: true, error: `大模型改稿失败，未生成新版本：${safeError(styled.error)}` };
  return { blocked: false, script: styled.script };
}
